...
class apt_rent_monitor:
	...
	def __init__(self):
		aptName = ["Archerstone", "Alborada", "Presidio", "Mission Peaks", "Mission Peaks II"]
		aptUrl = ["http://www.equityapartments.com/california/san-francisco-bay-apartments/fremont/archstone-fremont-center-apartments.aspx",
			  ...
			  "https://www.on-site.com/web/online_app/choose_unit?goal=6&attr=5&property_id=144840",
...
#		pattern2 = re.compile('<td class="addr_or_apt" title="(.*?)".*?<span class="internal_rent">(.*?)</span>.*?data-available-date="(.*?)".*?data-floorplan-description="1 bed', re.S)	
		pattern2 = re.compile('<td class="addr_or_apt" title="(.*?)".*?<span class="internal_rent">(.*?)</span>.*?data-available-date="0(.*?)"', re.S)       
		pattern3 = re.compile('{"unit_id":.*?"unit_type_floor_plan_name":"(.*?)","make_ready_date":.*?,"rent":(.*?),".*?"beds":"1.0".*?"available_date":"(.*?)"', re.S)
		
		self.moveInDate = "08-15-2015"
		self.aptInfo = []
		self.availableRooms = []
		for i in range(2):
			self.aptInfo.append([aptName[i], aptUrl[i], pattern1])
                for m in range(2,3):
                        self.aptInfo.append([aptName[m], aptUrl[m], pattern2])
		for j in range(3,5):
			aptUrl[j] = aptUrl[j] + "/" + self.moveInDate
			self.aptInfo.append([aptName[j], aptUrl[j], pattern3])
